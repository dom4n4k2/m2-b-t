...
def restart_sequence(worm_counter):
...
frame_ever = cv2.imread('final_circle.png',-1)
...
worm_counter = restart_sequence(worm_counter)
# LOOP!
while True:
    current_time = actual_time()
    ...
    for c in cnts:
        ...
        if cv2.contourArea(c) > MIN_SIZE_FOR_MOVEMENT:
            ...
            if ((diagonal_of_rectangle > 100) & (diagonal_of_rectangle < 200) & (points_check_distance < 66)):
                fishing_itself_last_recognise = actual_time()
                if (abs(x_check - previous_x_check) <= 3):
                    dif_x_counter = dif_x_counter + 1
                elif (abs(x_check - previous_x_check) > 3):
                    dif_x_counter = 0
                    previous_x_check = x_check

                if (abs(y_check - previous_y_check) <= 3):
                    dif_y_counter = dif_y_counter + 1
                elif (abs(y_check - previous_x_check) > 3):
                    dif_y_counter = 0
                ...
                if (((tt - ts > tc) or (tt - ts < 0))& (dif_x_counter > 1 ) & (dif_y_counter > 1)):
                    pydirectinput.click(int(x_check), int(y_check))
                    fishing_cooldown_time = actual_time()


    if(image_check_counter > 100):
        c1, c2, c3 = image_colors(495, 430, 25, 10)

        if ((c1 < 7600) or (c1 > 7700)) & ((c2 <  7600) or (c2 > 7700)) & ((c3 < 850) or (c3 > 950)):
            time.sleep(5)
            worm_counter = restart_sequence(worm_counter)
        image_check_counter = 0

    image_check_counter = image_check_counter + 1
