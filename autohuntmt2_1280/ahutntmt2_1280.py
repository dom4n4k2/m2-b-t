...
from autohuntmt2_1280.data_struct import *
...
screen_1_click = ( data_struct['screen_1_click_x'],  data_struct['screen_1_click_y'])
screen_2_click = ( data_struct['screen_1_click_x'] + x_resolution,  data_struct['screen_1_click_y'])
hp_count = 0
hp_wp_50_60 = ( data_struct['hp_wp_50_60_v1'],  data_struct['hp_wp_50_60_v2'],
               data_struct['hp_wp_50_60_width'], data_struct['hp_wp_50_60_height'])
mp_wp_50_60 = ( data_struct['mp_wp_50_60_v1'],  data_struct['mp_wp_50_60_v2'],
...
def if_statement(c1, c2, c3, c1_min, c1_max, c2_min, c2_max, c3_min, c3_max, function, mode, *args):
    if(mode == 'hp'):
        if (c1 > c1_min) & (c1 < c1_max) & (c2 > c2_min) & (c2 < c2_max) & (c3 > c3_min) & (c3 < c3_max):
            ...
            if (hp_count > 20):
                #resp_click
                time.sleep(10)
                pydirectinput.click( data_struct['resp_click_x'],  data_struct['resp_click_y'])
                hp_count = 0
                time.sleep(1)
                heal_hp()
                time.sleep(1)
                ...
                time.sleep(5)
                #autolowy
                pydirectinput.click( data_struct['autolowy_autolowy_x'],  data_struct['autolowy_autolowy_y'])
                time.sleep(1)
                #atak
                pydirectinput.click( data_struct['autolowy_atak_x'],  data_struct['autolowy_atak_y'])
                time.sleep(0.25)
                pydirectinput.click( data_struct['autolowy_focus_x'],  data_struct['autolowy_focus_y'])
                time.sleep(0.25)
                # exit
                pydirectinput.click( data_struct['autolowy_exit_x'],  data_struct['autolowy_exit_y'])
                time.sleep(0.25)
                start_skills("dupa")
    ...
    if (mode =='logout'):
        if (c1 > c1_min) & (c1 < c1_max) & (c2 > c2_min) & (c2 < c2_max) & (c3 > c3_min) & (c3 < c3_max):
            print('is logged out, bla bla bla')
            function()
    if (mode =='is_logged'):
        if (c1 > c1_min) & (c1 < c1_max) & (c2 > c2_min) & (c2 < c2_max) & (c3 > c3_min) & (c3 < c3_max):
            return False
...
def start_skills(*args):
    ...
    #menuuu
    if len(args)== 0:
        pydirectinput.click( data_struct['autolowy_menu_x'],  data_struct['autolowy_menu_y'])
        time.sleep(1)
        #autolowy
        pydirectinput.click( data_struct['autolowy_autolowy_x'],  data_struct['autolowy_autolowy_y'])
        time.sleep(1)
        #
        pydirectinput.click( data_struct['autolowy_start_x'],  data_struct['autolowy_start_y'])
        time.sleep(0.25)
        #focus
        pydirectinput.click( data_struct['autolowy_focus_x'],  data_struct['autolowy_focus_y'])
        time.sleep(0.25)
...
def logout_check(mode):
    ...
    w1, w2, w3 = image_colors( data_struct['logout_v1'],  data_struct['logout_v2'],
